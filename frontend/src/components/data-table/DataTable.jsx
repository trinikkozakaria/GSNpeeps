...
export const DataTable = ({ caption, columns, rows, rowKey, emptyMessage }) => {
  ...
  return (
    <>
      <div
        className="hidden overflow-x-auto rounded-xl border border-white/10 md:block"
        role="region"
        aria-label={caption}
        tabIndex={0}
      >
        <table className="min-w-full divide-y divide-white/10 text-left text-sm">
          ...
          <tbody className="divide-y divide-white/10">
            {rows.map((row) => (
              <tr key={rowKey(row)} className="bg-slate-900/40">
                {columns.map((column) => (
      ...
      <ul className="grid gap-3 md:hidden" aria-label={caption}>
        {rows.map((row) => (
          <li key={rowKey(row)} className="rounded-xl border border-white/10 bg-slate-900/40 p-4">
            <dl className="grid gap-2">
              {columns.map((column) => (
                <div key={column.key} className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <dd className="text-sm text-slate-100">{column.render(row)}</dd>
                </div>
              ))}
            </dl>
          </li>
