...
export const ApprovalTimeline = ({ history }) => {
  ...
  return (
    <ol className="relative border-l border-slate-900/15 pl-5">
      {history.map((entry, index) => (
        <li key={`${entry.decided_at}-${index}`} className="mb-5 last:mb-0">
          <span
            aria-hidden="true"
            className="absolute -left-1.5 mt-1.5 h-3 w-3 rounded-full bg-cyan-700"
          />
          ...
          {entry.catatan && <p className="mt-2 text-sm text-slate-600">{entry.catatan}</p>}
        </li>
      ))}
    </ol>
  );
