...
export const uploadFeedAttachmentRequest = async (id, file) => {
  const form = new FormData();
  form.append("file", file);
  return data(await apiClient.post(`/company-feed/${id}/attachments`, form));
};
export const deleteFeedAttachmentRequest = async (feedId, attachmentId) =>
  data(await apiClient.delete(`/company-feed/${feedId}/attachments/${attachmentId}`));
