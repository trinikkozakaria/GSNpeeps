...
func New(
	...
	if uat.Handler != nil {
		...
		api.Handle("/company-feed/{id}/attachments", protected(uat.Handler.UploadFeedAttachment)).Methods(http.MethodPost)
		api.Handle("/company-feed/{id}/attachments/{attachmentId}", protected(uat.Handler.DeleteFeedAttachment)).Methods(http.MethodDelete)
